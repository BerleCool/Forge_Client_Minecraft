...
with zipfile.ZipFile(jar) as archive:
    ...
    if (mod['modid'], mod['mcversion'], mod['version']) != ('forgeclient', '1.8.9', '0.3.0-alpha'):
...
print(json.dumps({'jar': str(jar), 'bytes': jar.stat().st_size, 'sha256': hashlib.sha256(jar.read_bytes()).hexdigest(), 'java_major': 52, 'classes': classes, 'live_modules': 89, 'placeholder_modules': 0, 'mod': mod}, indent=2))
