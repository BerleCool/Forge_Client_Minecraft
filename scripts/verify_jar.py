#!/usr/bin/env python3
"""Validate a distributable, never mistake the UI preview/dev JAR for the mod."""
import hashlib
import json
import pathlib
import struct
...
print(json.dumps({'jar': str(jar), 'bytes': jar.stat().st_size, 'sha256': hashlib.sha256(jar.read_bytes()).hexdigest(), 'java_major': 52, 'classes': classes, 'mod': mod}, indent=2))
