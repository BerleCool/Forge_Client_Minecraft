...
public final class ModuleCatalog {
    ...
    public static ModuleRegistry create() {
        ...
        hud(r,"compass","Direction HUD","Find your heading","An eight-direction heading and your yaw. No entity scanning or hidden player information.",false,.5,.018);
        ...
        hud(r,"clock","Clock","Real-world time","Uses your device's local time zone, sampled four times per second.",false,.988,.15)
            .add(Setting.bool("twenty_four","24-hour time","Use HH:mm instead of a 12-hour clock.",true));
        hud(r,"session","Playtime","Time in this world","A monotonic session timer, reset on world changes. Does not depend on the system clock remaining unchanged.",false,.012,.30);
        hud(r,"server","Server Address","Where you are playing","Shows the current server name, not its resolved IP. Streamer mode replaces identifying text.",false,.5,.075);
        ...
        hud(r,"day_time","Day Counter","Time under the Minecraft sun","The world's day number and time of day. Display only; never changes server time.",false,.5,.13);
        hud(r,"hit_distance","Reach Display","A local hit-distance readout","Distance from your eyes to the selected hit point on a local attack. This is NOT server-confirmed reach and does not extend attacks.",false,.5,.79)
            .add(Setting.number("timeout","Display seconds","Hide the reading after this many seconds.",3,1,10,1));
        hud(r,"inventory_counts","Item Counter","Arrows, pearls and blocks","Counts your own inventory every five ticks. Useful for PvP and building without constantly opening your inventory.",false,.30,.94);
        hud(r,"pack_info","Pack Display","Your active pack","The names of your enabled resource packs. Uses Minecraft's existing repository; never reads arbitrary files.",false,.012,.50);
        ...
        hud(r,"waypoint","Waypoints","A place worth remembering","Distance and heading to your nearest user-created waypoint in this world and dimension. Manage points with /forgeclient waypoint.",false,.5,.20);
        ...
        r.add(module("fullbright","Lighting",Category.VISUAL,"Lighten the darkest corners","A reversible local gamma override. Disabled by default; check the rules of the server you play on.",false)
            .add(Setting.number("gamma","Gamma level","Restores the previous gamma when disabled.",10,1,16,.5)));
        r.add(module("fov_stabilizer","FOV",Category.VISUAL,"Keep the view consistent","Removes movement- and bow-induced FOV changes through Forge's FOV event. Does not change movement speed or attack mechanics.",false));
        ...
        r.add(module("particle_budget","Particle Changer",Category.PERFORMANCE,"Less particle work","Leases vanilla's particle setting. This is a real density reduction, not a custom GPU particle engine.",false)
        ...
        r.add(module("toggle_sprint","Toggle Sneak/Sprint",Category.UTILITY,"One tap, keep moving","While enabled, tap your vanilla sprint key to toggle sprint intent. Respects hunger, collisions, sneaking and item use. Stops on focus loss, menus and world changes.",false));
