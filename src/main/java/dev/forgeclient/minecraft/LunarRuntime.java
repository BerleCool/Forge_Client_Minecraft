...
public final class LunarRuntime {
    ...
    @SubscribeEvent public void tick(TickEvent.ClientTickEvent event){
        ...
        if((ticks%10)==0){if(enabled("lunar_light_overlay"))sampleLowLight();else lowLight.clear();if(enabled("lunar_item_tracker"))sampleInventoryDelta();}
    ...
    private void resetWorldState(){trail.clear();lowLight.clear();cuiOne=cuiTwo=null;combo=0;comboTarget=null;lastAttackNs=lastPearlNs=comboAtNs=0;lastActionBar="";previousInventory=Collections.emptyMap();lastInventoryDelta="NO RECENT CHANGE";lastHurtTime=0;latestKnockback=0;lastX=lastZ=lastSpeed=0;}
    ...
    private void sampleHud(long n){
        ...
        if(enabled("lunar_potion_counter")){int potions=0;for(ItemStack s:p.inventory.mainInventory)if(s!=null&&s.getItem()==Items.potionitem)potions+=s.stackSize;client.telemetry.put("lunar_potion_counter",potions+" POTIONS");}
        ...
        if(enabled("lunar_minimap"))client.telemetry.put("lunar_minimap",minimapRows());if(enabled("lunar_tier"))sampleTier();if(enabled("lunar_actionbar"))client.telemetry.put("lunar_actionbar",lastActionBar.isEmpty()?"ACTION BAR --":lastActionBar);
    }
    private int countItem(net.minecraft.item.Item item){int n=0;for(ItemStack s:mc.thePlayer.inventory.mainInventory)if(s!=null&&s.getItem()==item)n+=s.stackSize;return n;}
    ...
    private void sampleTnt(){EntityTNTPrimed best=null;double dist=Double.MAX_VALUE;for(Object raw:mc.theWorld.loadedEntityList)if(raw instanceof EntityTNTPrimed){EntityTNTPrimed t=(EntityTNTPrimed)raw;double d=mc.thePlayer.getDistanceSqToEntity(t);if(d<dist){dist=d;best=t;}}client.telemetry.put("lunar_tnt",best==null?"NO PRIMED TNT":String.format(Locale.ROOT,"TNT %.2f s / %.1f m",best.getFuse()/20D,Math.sqrt(dist)));}
    ...
    private String[] scoreboardLines(int max){if(!play())return new String[]{"NO SCOREBOARD"};Scoreboard sb=mc.theWorld.getScoreboard();ScoreObjective obj=sb.getObjectiveInDisplaySlot(1);if(obj==null)return new String[]{"NO SCOREBOARD"};List<String> rows=new ArrayList<String>();rows.add(obj.getDisplayName());for(Score score:sb.getSortedScores(obj)){String name=score.getPlayerName();if(name==null||name.startsWith("#"))continue;rows.add(ScorePlayerTeam.formatPlayerName(sb.getPlayersTeam(name),name));}if(rows.size()>max+1)rows=rows.subList(rows.size()-(max+1),rows.size());return rows.toArray(new String[rows.size()]);}
    private String scoreboardPlain(){StringBuilder out=new StringBuilder();for(String line:scoreboardLines(15)){String p=EnumChatFormatting.getTextWithoutFormattingCodes(line);if(p!=null)out.append(p).append('\n');}return out.toString();}
    private boolean hypixel(){return mc.getCurrentServerData()!=null&&mc.getCurrentServerData().serverIP!=null&&mc.getCurrentServerData().serverIP.toLowerCase(Locale.ROOT).contains("hypixel");}
    ...
    private void sampleSkyblockAddons(){if(!hypixel()||!scoreboardPlain().toLowerCase(Locale.ROOT).contains("skyblock")){client.telemetry.put("lunar_sba","SKYBLOCK NOT DETECTED");return;}client.telemetry.put("lunar_sba",lastActionBar.isEmpty()?"SKYBLOCK ACTIVE":lastActionBar,String.format(Locale.ROOT,"HP %.1f",mc.thePlayer.getHealth()));}
    private void sampleNeu(){ItemStack held=mc.thePlayer.getHeldItem();if(held==null){client.telemetry.put("lunar_neu","NO HELD ITEM");return;}List<String> tip=held.getTooltip(mc.thePlayer,false),rows=new ArrayList<String>();rows.add(held.getDisplayName());for(int i=1;i<tip.size()&&rows.size()<4;i++)if(tip.get(i)!=null&&!tip.get(i).trim().isEmpty())rows.add(tip.get(i));client.telemetry.put("lunar_neu",rows.toArray(new String[rows.size()]));}
    ...
    private String[] minimapRows(){int r=mod("lunar_minimap").setting("radius").integer();List<String> rows=new ArrayList<String>();BlockPos origin=new BlockPos(mc.thePlayer.posX,mc.thePlayer.posY-1,mc.thePlayer.posZ);rows.add("N");for(int dz=-r;dz<=r;dz++){StringBuilder line=new StringBuilder();for(int dx=-r;dx<=r;dx++){if(dx==0&&dz==0){line.append('@');continue;}BlockPos pos=origin.add(dx,0,dz);if(!mc.theWorld.isBlockLoaded(pos)){line.append('?');continue;}Material mat=mc.theWorld.getBlockState(pos).getBlock().getMaterial();line.append(mat==Material.water?'~':mat==Material.lava?'!':mat==Material.air?'.':'#');}rows.add(line.toString());}return rows.toArray(new String[rows.size()]);}
    private void sampleInventoryDelta(){Map<String,Integer> current=new HashMap<String,Integer>();for(ItemStack s:mc.thePlayer.inventory.mainInventory)if(s!=null){String key=s.getItem().getUnlocalizedName();Integer old=current.get(key);current.put(key,(old==null?0:old)+s.stackSize);}if(!previousInventory.isEmpty()){Set<String> keys=new HashSet<String>();keys.addAll(previousInventory.keySet());keys.addAll(current.keySet());for(String key:keys){int before=previousInventory.containsKey(key)?previousInventory.get(key):0,after=current.containsKey(key)?current.get(key):0;if(after!=before){lastInventoryDelta=(after>before?"+":"")+(after-before)+" "+cleanItemKey(key);break;}}}previousInventory=current;}
