...
public final class TelemetrySampler {
    ...
    private void sample(long now){
        ...
        if(enabled("lunar_inventory")){Telemetry.Item[] hotbar=new Telemetry.Item[9];for(int i=0;i<9;i++){ItemStack stack=p.inventory.mainInventory[i];if(stack!=null)hotbar[i]=item(stack,false);}data.hotbar=hotbar;data.hotbarSlot=Math.max(0,Math.min(8,p.inventory.currentItem));}
